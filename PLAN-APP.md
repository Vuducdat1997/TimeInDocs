...
- [x] **APP-1.7** Base giao diện dùng chung: `lib/theme/app_nav_bar.dart` (`TimeInAppBar` + `TimeInAction` + `initialsOf`) thay mọi `AppBar` tự dựng, `lib/theme/app_colors.dart` thay màu viết trực tiếp. Quy tắc và checklist màn mới ghi ở `docs/UI-BASE.md`, trỏ từ `App/AGENTS.md`.
- [x] **APP-3.6** Ô **“Ghi nhớ đăng nhập”** ở màn đăng nhập, mặc định bật. Tắt thì token chỉ nằm trong RAM, thoát app phải đăng nhập lại; lựa chọn được nhớ cho lần sau.

**Đầu vào:** `PLAN-BE.md` BE-1.5 (`/health`) và BE-1.4 (biến môi trường); ảnh thiết kế và logo do chủ dự án cung cấp.
**Đầu ra:** `lib/main.dart`, `lib/auth/login_page.dart`, `lib/theme/app_colors.dart`, `lib/theme/app_nav_bar.dart`, `assets/branding/`, `scripts/make-icons.sh`, `docs/design/trang-chu/`, `docs/UI-BASE.md`.
**Nghiệm thu:** chạy trên iOS Simulator gọi được `/health`; `flutter analyze` sạch lỗi; đổi `API_BASE_URL` bằng `--dart-define` hoạt động trên máy thật; thiết kế mở được từ link trong plan; icon hiển thị đúng trên màn hình chính của máy ảo; mọi màn dùng `TimeInAppBar`.
...
**Chưa làm ở màn này:** đồng hồ trên màn cập nhật theo lần vẽ chứ chưa tự nhích mỗi phút (tránh `Timer.periodic` làm widget test không kết thúc); chuông thông báo nằm trên thanh điều hướng và mới chỉ hiển thị, chưa mở được (chờ BE-9.6).
